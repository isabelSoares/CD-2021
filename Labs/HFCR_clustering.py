...
featured_datas_scaled = prepfunctions.mask_feature_selection(datas_scaled.copy(), 'DEATH_EVENT', False, './Results/FeatureSelection/HFCR Feature Selection - Features')

scaling_fs_pca = [(False, False, False), (True, False, False), (True, True, False), (True, True, True)]
...
for key in datas_scaled:
	if key != 'Original':
		break
	for scaling, fs, pca in scaling_fs_pca:
		if (scaling, fs, pca) == (False, False, False):
			data = original_data
			subDir = graphsDir + 'Original/'
			if not os.path.exists(subDir):
				os.makedirs(subDir)
		elif (scaling, fs, pca) == (True, False, False):
			data = datas_scaled[key].copy()
			subDir = graphsDir + 'Scaling/'
			if not os.path.exists(subDir):
				os.makedirs(subDir)
		elif (scaling, fs, pca) == (True, True, False):
			data = featured_datas_scaled[key].copy()
			subDir = graphsDir + 'Scaling and Feature Selection/'
			if not os.path.exists(subDir):
				os.makedirs(subDir)
		elif (scaling, fs, pca) == (True, True, True):
			data = featured_datas_scaled[key].copy()
			subDir = graphsDir + 'Scaling, Feature Selection and PCA/'
			if not os.path.exists(subDir):
				os.makedirs(subDir)
			print()
			print()
			print('Scaling = ' + str(scaling) + '; Feature Selection = ' + str(fs) + ': PCA = ' + str(pca))
			print()
