...
for dt in all_datas:
    ...
    for train_i, test_i in skf.split(X, y):
        ...
        for category in ['Original', 'UnderSample', 'OverSample', 'SMOTE']:
            ...
            labels_values_fs[category] = trn_data_fs[category].columns.values
            # Test
    ...
    final_lst_fs = {}
    for category in ['Original', 'UnderSample', 'OverSample', 'SMOTE']:
        final_lst_b[category] = [trn_x_b_lst[category], trn_y_b_lst[category], tst_x_b_lst[category], tst_y_b_lst[category], labels_values[category].copy()]
        final_lst_fs[category] = [trn_x_fs_lst[category], trn_y_fs_lst[category], tst_x_fs_lst[category], tst_y_fs_lst[category], labels_values_fs[category].copy()]
...
for key in ['Original', 'UnderSample', 'OverSample', 'SMOTE']:
    ...
    for dt in range(len(all_datas_splits)):
        ...
        labels_category = all_datas_splits[dt][key][4]
        if(last_name == ' - Scaling' and offset == 1):
            ...
            labels_category = datas_splits_scaling_featureselection[key][4]
        ...
        labels_features = list(labels_category)
